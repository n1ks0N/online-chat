    ...
    if (send) {
      ...
      req.onreadystatechange = () => {
        if (req.readyState == XMLHttpRequest.DONE) {
          ...
          let arr = result['directions']['links'].slice()
          if (arr.length >= 20) {
            arr.pop();
          }
          arr.unshift(data.directions.links[0]);
          result.directions.links = arr;
            ...
            <div className="links__wrapper">
              {!!data &&
                data.directions.links.map((data, i) => (
                  <div className="links__wrapper__item links__wrapper_link_margin" key={i}>
                    <a
                      href={`//${data.link}`}
                      onClick={() =>
