  ...
  const sendMessage = () => {
    ...
    setData((prev) => {
      if (allow) {
        ...
        inputMessageValue.replace(/((?:https?:\/\/|ftps?:\/\/|\bwww\.)(?:(?![.,?!;:()]*(?:\s|$))[^\s]){2,})|(\n+|(?:(?!(?:https?:\/\/|ftp:\/\/|\bwww\.)(?:(?![.,?!;:()]*(?:\s|$))[^\s]){2,}).)+)/gim, (m, link, text) => {
          ...
          <div className="main">
            <div className="chat">
                  ...
                  data.directions.chat.map((data, i) => (
                    <div className="chat__message" key={i}>
                      ...
                      <div className="chat__body">
                        <p className="chat__text" dangerouslySetInnerHTML={{ __html: data.message }} />
                        <span>{data.date.split(' ').splice(0, 5).join(' ')}</span>&nbsp;
                      <span>#{i}</span>
                      </div>
                    </div>
                  ))
              ...
              <div className="chat__form">
                ...
                <button type="button" className="btn btn-success" onClick={sendMessage}>Отправить</button>
              </div>
            </div>
          </div>
